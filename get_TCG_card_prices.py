...
def ABBV_RARITY(r):
    abbv = ''
    r = r.lower()
    # Assign the result of replace back to r
    r = r.replace('secret rare', 's e')
    ...
    if response.status_code == 200:
        ...
        for card in cards:
            for card_set in card.get('card_sets', []):
                # Filter to only include the current set
                if set_name in card_set['set_name']:
                    record = (
                        card['name'],
                        ABBV_RARITY(card_set.get('set_rarity', 'N/A')),
                        card_set.get('set_edition', 'N/A'),
                        # Assuming 'num' is the card's ID
                        card_set['set_code'],
                        card_set.get('set_price', 'N/A'),
                        card.get('card_prices', [])[0].get(
                            'cardmarket_price', 'N/A'),
                        card_set['set_name'],
